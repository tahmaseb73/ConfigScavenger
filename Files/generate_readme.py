...
def main():
    ...
    for i in range(8):  # تا Sub8.txt
        ...
        if os.path.exists(sub_file):
            sub_links.append(f"https://raw.githubusercontent.com/tahmaseb73/v2ray-configs/main/Sub{i+1}.txt")
...
"""
    for protocol in protocols:
        ...
        if count > 0:
            readme_content += f"| {protocol.capitalize()} | {count} | <a href=\"https://raw.githubusercontent.com/tahmaseb73/v2ray-configs/main/Splitted-By-Protocol/{protocol}.txt\" onclick=\"navigator.clipboard.writeText(this.href);alert('لینک کپی شد!')\">`{protocol}.txt`</a> |\n"
...
"""
    ...
    for protocol in protocols:
        if protocol_counts.get(protocol, 0) > 0:
            readme_content += f"**{protocol_names_fa.get(protocol, protocol.capitalize())}:**  \n<a href=\"https://raw.githubusercontent.com/tahmaseb73/v2ray-configs/main/Splitted-By-Protocol/{protocol}.txt\" onclick=\"navigator.clipboard.writeText(this.href);alert('لینک کپی شد!')\">`{protocol}.txt`</a>\n\n"
...
"""
    for country in sorted(country_counts.keys()):
        ...
        if count > 0:
            ...
            readme_content += f"| {flag} {country} ({country_names_fa[country]}) | {count} | <a href=\"https://raw.githubusercontent.com/tahmaseb73/v2ray-configs/main/Countries/{country}.txt\" onclick=\"navigator.clipboard.writeText(this.href);alert('لینک کپی شد!')\">`{country}.txt`</a> |\n"
